...
void MainWindow::showResult(const std::vector<SkillNode*>& skills,
    ...
    if (!instakill && buffs.empty())
        skillText += QString("<small style='color:#6c7086;'>합계: %1</small>")
                         .arg(runningTotal, 0, 'f', 1);
    ...
    if (!instakill) {
        ...
        if (buffs.empty()) {
            ...
            for (auto* b : buffs) {
                buffText += QString("• %1  (%2)").arg(
                    QString::fromStdString(b->bufferName),
                    QString::fromStdString(b->description)) + "<br>";
            }
            buffText += QString(
